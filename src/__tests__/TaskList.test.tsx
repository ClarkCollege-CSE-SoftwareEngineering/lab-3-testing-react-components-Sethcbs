...
describe('TaskList', () => {
  ...
  //Comment/question I will hopefully remember to bring up in class. I
  // did not realize how in depth you had to go to 'recreate' a user
  // interacting with the webpage. I tried to copy what was happening in
  // 'displaying tasks' and didn't realize how differently these functions
  // were being handled. If I understand correctly, you don't need as much
  // set up to test loading the tasks because that's an 'automatic'
  // action. The difference here is that you need to open the page,
  // type in the new task, click add task, then see the resulting 
  // error message.

  describe('creating a task', () => {
    it('shows error message when create task fails', async () => {
      const user = userEvent.setup();
      mockedTaskApi.fetchTasks.mockResolvedValue([]);
      ...
      await user.type(screen.getByLabelText(/task title/i), 'hi there');
      await user.click(screen.getByRole('button', { name: /add task/i }));
